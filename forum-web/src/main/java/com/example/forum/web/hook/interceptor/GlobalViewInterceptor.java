        ...
        if (handler instanceof HandlerMethod) {
            ...
            if (ReqInfoContext.getReqInfo() == null || ReqInfoContext.getReqInfo().getUserId() == null) {
                if (handlerMethod.getMethod().getAnnotation(ResponseBody.class) != null
                    ...
                    // 访问需要登录的页面时，直接跳转到登录界册"
                    response.sendRedirect("/");
                }
                return false;
            }
            if (permission.role() == UserRole.ADMIN && UserRole.ADMIN.getCode() != ReqInfoContext.getReqInfo().getUser().getRole()) {
                // 设置为无权限
        ...
        if (!ObjectUtils.isEmpty(modelAndView)) {
            if (response.getStatus() != HttpStatus.OK.value()) {
                try {
                    ...
                    // fixme 对于异常重定向到 /error 时，会导致登录信息丢失，待解册"
