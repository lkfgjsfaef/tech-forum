...
import com.example.forum.core.autoconf.DynamicConfigContainer;
import com.example.forum.core.dal.DsAno;
import com.example.forum.core.dal.DsSelectExecutor;
import com.example.forum.core.dal.MasterSlaveDsEnum;
import com.example.forum.core.permission.Permission;
import com.example.forum.core.permission.UserRole;
import com.example.forum.core.senstive.SensitiveService;
import com.example.forum.core.util.EmailUtil;
...
import com.example.forum.service.statistics.converter.StatisticsConverter;
...
import com.example.forum.service.statistics.service.impl.CountServiceImpl;
import com.example.forum.web.front.login.wx.helper.WxLoginQrGenIntegration;
import com.example.forum.web.front.test.vo.EmailReqVo;
...
public class TestController {
    ...
    public ResVo<String> email(EmailReqVo req) {
        ...
        if (StringUtils.isBlank(req.getTitle())) {
            req.setTitle("Tech Forum的测试邮件发送");
        }
        if (StringUtils.isBlank(req.getContent())) {
            req.setContent("Tech Forum的测试发送内容");
        } else {
            // 测试邮件内容，不支持发送邮件正文，避免出现垃圾情况
            req.setContent(StringEscapeUtils.escapeHtml4(req.getContent()));
        }

        EmailUtil.sendSimpleEmail(req.getTo(), req.getTitle(), req.getContent());
        boolean ans = true;
    ...
    public AISourceEnum updateAi(String ai) {
        ...
        return AISourceEnum.valueOf(ai);
    }
        ...
        try (ExcelWriter excelWriter = FastExcel.write(outputStream, RequestCountExcelDO.class).build()) {
            for (int sheetIndex = 0; sheetIndex < sheetCount; sheetIndex++) {
                ...
                for (int pageIndex = 0; pageIndex < sheetSize / pageSize; pageIndex++) {
                    // 第一页是从0-9999，第二页是从 10000-19999
                    int offset = sheetIndex * sheetSize + pageIndex * pageSize + 1;
                    // TODO 自定义线程池+ CountDownLatch 进行优化
                    List<RequestCountDO> data = requestCountService.listRequestCount(PageParam.newPageInstance((long) offset, (long) pageSize));
    ...
    public Object refreshToken() {
        ...
        return MapUtils.of("old", old, "new", newToken);
    }
