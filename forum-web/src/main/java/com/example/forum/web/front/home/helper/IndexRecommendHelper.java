package com.example.forum.web.front.home.helper;
...
import com.example.forum.core.async.AsyncUtil;
import com.example.forum.core.common.CommonConstants;
...
public class IndexRecommendHelper {
    ...
    public IndexVo buildIndexVo(String activeTab, Long page) {
        ...
        AsyncUtil.concurrentExecutor("首页响应")
                .async(() -> vo.setArticles(articleList(category.getCategoryId(), page)), "文章列表")
                .async(() -> vo.setTopArticles(topArticleList(category)), "置顶文章")
                .async(() -> vo.setHomeCarouselList(homeCarouselList()), "轮播册")
                .async(() -> vo.setSideBarItems(sidebarService.queryHomeSidebarList()), "侧边册")
    ...
    private List<CarouseDTO> homeCarouselList() {
        ...
        return configList.stream()
                .map(configDTO -> {
                    CarouseDTO dto = new CarouseDTO();
                    dto.setName(configDTO.getName());
                    dto.setImgUrl(configDTO.getBannerUrl());
                    dto.setActionUrl(configDTO.getJumpUrl());
                    return dto;
                })
     ...
     * 文章列表
     */
    private PageListVo<ArticleDTO> articleList(Long categoryId, Long page) {
        PageParam pageParam = PageParam.newPageInstance(page, PageParam.DEFAULT_PAGE_SIZE);
        return articleService.queryArticlesByCategory(categoryId, pageParam);
    }
