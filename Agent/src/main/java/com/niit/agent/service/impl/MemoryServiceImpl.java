...
public class MemoryServiceImpl implements MemoryService {
    ...
    public List<Map<String, Object>> buildContext(Long sessionId) {
        ...
        if (!latestUserQuestion.isEmpty()) {
            performRagRetrieval(latestUserQuestion, context);
        }
    ...
    private void performRagRetrieval(String query, List<Map<String, Object>> context) {
        try {
            log.info("执行RAG混合检索: query: {}", query);
            ...
            // 2. BM25 Keyword Search (Literal) using RediSearch
            ...
            if (!combinedResults.isEmpty()) {
                ...
                if (rerankService != null) {
                    finalResults = rerankService.rerank(query, combinedResults, 3);
                } else {
